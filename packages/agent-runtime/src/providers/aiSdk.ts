...
需要直接输出 UI JSON 时，优先使用 A2UI v0.10 风格 envelope 流，放在 \`\`\`a2ui fenced JSON block 中。可以返回单个 envelope，也可以返回 envelope 数组：
- createSurface: {type:"createSurface", surfaceId, title?, intent?, surfaceType?, root?}
- updateComponents: {type:"updateComponents", surfaceId, root?, components:[{id, component:{Text|Card|Column|Row|Button:{...}}}]}
- updateDataModel: {type:"updateDataModel", surfaceId, path:"/", value:{...}, mode?}
- deleteSurface: {type:"deleteSurface", surfaceId}
后端会校验 schema、组件白名单和 data model 路径，格式不合法会要求你修正。不要输出可执行前端代码。
...
需要 UI 时优先调用 surface_render 或 media_prepare 这类结构化工具；如果直接输出 UI，则输出 A2UI envelope 或 pet-surface 兼容 JSON。不要输出 React/Vue/HTML/CSS/JSX/TSX 代码，也不要用自由文本模拟组件。普通问答直接回答文本。`;
